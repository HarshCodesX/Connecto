        ...
        <div className="flex gap-5 pb-5">
            ...
            {/* Story Card */}
            {
                stories.map((story, index) => (
                    <div onClick={() => setViewStory(story)} className={`relative rounded-lg shadow min-w-30 max-w-30 max-h-40 cursor-pointer hover:shadow-lg transition-all duration-200 bg-gradient-to-b from-indigo-500 to-purple-600 hover:from-indigo-700 hover:to-purple-800 active:scale-95`} key={index}>
                        <img className="absolute size-8 top-3 left-3 z-10 rounded-full ring ring-gray-100 shadow" src={story.user.profile_picture} alt="user-profile" />
                                ...
                                <div className="absolute inset-0 z-1 rounded-lg bg-black overflow-hidden">
                                    {
                                        ...
                                        <img src={story.media_url} alt="story-image" className="h-full w-full object-cover hover:scale-110 transition duration-500 opacity-70 hover:opacity-80" />
                                        :
