...
import { addMessage, fetchMessages, resetMessage } from '../features/messages/messagesSlice.js';
    ...
    <div className='flex flex-col h-screen'>
            ...
            messages.toSorted((a,b) => new Date(a.createdAt) - new Date(b.createdAt)).map((message, index) => (
              <div className={`flex flex-col ${message.to_user_id !== user._id ? 'items-start' : 'items-end'}`} key={index}>
                <div className={`p-2 text-sm max-w-sm bg-white text-slate-700 rounded-lg shadow ${message.to_user_id !== user._id ? 'rounded-bl-none' : 'rounded-br-none'}`}>
                  {
                    message.message_type === 'image' && <img src={message.media_url} className='w-full max-w-sm rounded-lg mb-1' alt="user-image" />
                  }
                  <p>{message.text}</p>
                </div>
              </div>
      ...
      <div className='px-4'>
          <div className='flex items-center gap-3 pl-5 p-1.5 bg-white w-full max-w-xl mx-auto border border-gray-200 shadow rounded-full mb-5'>
            ...
            <label htmlFor="image">
              {
                image ? <img className='h-8 rounded' alt='image' src={URL.createObjectURL(image)} /> : <ImageIcon className='size-7 text-gray-400 cursor-pointer' />
              }
